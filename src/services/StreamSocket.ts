...
export default class StreamSocket {
  ...
  private onMessage = (message: unknown) => {
    const parse = () => JSON.parse(message.toString()) as AudioMessage;

    let parsed: AudioMessage;
    ...
    try {
      ...
      this.logger.error('Error handling parsed websocket message', { error });
      this.logger.error('Parsed message is1 %s', JSON.stringify(parsed));
    }
  };
}
